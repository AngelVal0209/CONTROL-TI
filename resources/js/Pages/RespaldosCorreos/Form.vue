<template>
  <div class="flex flex-col gap-4 max-w-3xl">
    <div class="flex items-center gap-2">
      <Button icon="pi pi-arrow-left" severity="secondary" text @click="router.get('/respaldos/correos')" />
      <h1 class="text-2xl font-bold">{{ respaldo ? 'Editar Respaldo de Correos' : 'Nuevo Respaldo de Correos' }}</h1>
    </div>
      ...
      <Card>
        ...
        <template #footer>
          <div class="flex justify-end gap-2">
            <Button label="Cancelar" severity="secondary" @click="router.get('/respaldos/correos')" />
...
function submit() {
  if (props.respaldo) {
    form.post(`/respaldos/correos/${props.respaldo.id}`, {
      _method: 'PUT',
      forceFormData: true,
    })
  } else {
    form.post('/respaldos/correos', {
      forceFormData: true,
    })
  }
}
