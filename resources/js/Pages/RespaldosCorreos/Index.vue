<template>
  <div class="flex flex-col gap-4">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold">Respaldos de Correos</h1>
      <Button label="Nuevo Respaldo" icon="pi pi-plus" @click="router.get('/respaldos/correos/create')" />
    </div>
    ...
    <DataTable :value="respaldos.data" :lazy="true" :totalRecords="respaldos.total" :rows="respaldos.per_page" :first="(respaldos.current_page - 1) * respaldos.per_page" paginator :rowsPerPageOptions="[10, 25, 50]" @page="onPage">
      ...
      <Column header="Archivo">
        <template #body="{ data }">
          <Button v-if="data.archivo" icon="pi pi-download" severity="info" text @click="router.get(`/respaldos/correos/${data.id}/download`)" />
      ...
      <Column header="Acciones" style="width: 150px">
        <template #body="{ data }">
          <div class="flex gap-1">
            <Button icon="pi pi-eye" severity="info" text @click="router.get(`/respaldos/correos/${data.id}`)" />
            <Button icon="pi pi-pencil" severity="secondary" text @click="router.get(`/respaldos/correos/${data.id}/edit`)" />
...
function applyFilters() {
  router.get('/respaldos/correos', filters, { preserveState: true, replace: true })
}

function onPage(event) {
  router.get('/respaldos/correos', { page: event.page + 1, ...filters }, { preserveState: true, replace: true })
}

function confirmDelete(respaldo) {
  confirm.require({
    message: `¿Eliminar este respaldo de correos?`,
    ...
    accept: () => {
      router.delete(`/respaldos/correos/${respaldo.id}`, {
